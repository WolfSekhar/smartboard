...
(function (root, factory) {
  ...
  const STYLES = `
    * { box-sizing: border-box; margin: 0; padding: 0; border-radius: 0px !important; }
    ...
    /* Dropdowns */
    .adw-select {
      width: 100%; height: 28px; background: var(--adw-btn-sec-bg); color: var(--adw-btn-sec-text);
      border: 1px solid var(--adw-card-border); font-size: 11px; font-weight: 500;
      padding: 0 6px; outline: none; cursor: pointer;
    }
    .adw-select:hover { border-color: var(--adw-accent-color); }

    /* Segmented Controls */
    .segmented-tabs { display: flex; width: 100%; border: 1px solid var(--adw-card-border); }
    .segmented-tab {
      flex: 1; height: 26px; background: var(--adw-btn-sec-bg); color: var(--adw-subtitle-color);
      border: none; border-right: 1px solid var(--adw-card-border);
      font-size: 10px; font-weight: 500; cursor: pointer; transition: all 0.15s ease;
    }
    .segmented-tab:last-child { border-right: none; }
    .segmented-tab.active { background: var(--adw-accent-color); color: #ffffff; }

    /* Action Buttons */
    .adw-action-btn {
      width: 100%; height: 28px; background: var(--adw-btn-sec-bg); color: var(--adw-text-color);
      border: 1px solid var(--adw-card-border); font-size: 11px; font-weight: 600;
      cursor: pointer; transition: all 0.15s ease; display: flex; align-items: center; justify-content: center; gap: 6px;
    }
    .adw-action-btn:hover { background: var(--adw-btn-sec-hover); border-color: var(--adw-accent-color); }

    /* Number Inputs */
    .adw-number-input {
      width: 60px; height: 24px; background: var(--adw-btn-sec-bg); color: var(--adw-text-color);
      border: 1px solid var(--adw-card-border); font-size: 11px; font-weight: 500;
      padding: 0 6px; text-align: right; outline: none; font-family: ui-monospace, monospace;
    }
    .adw-number-input:focus { border-color: var(--adw-accent-color); }

    /* Progress Scrubber */
    .adw-progress-container { width: 100%; display: flex; flex-direction: column; gap: 4px; padding: 4px 0; }
    .adw-progress-bar {
      width: 100%; height: 6px; -webkit-appearance: none; appearance: none;
      background: var(--adw-scale-track-bg); outline: none; cursor: pointer;
    }
    .adw-progress-bar::-webkit-slider-thumb {
  ...
  class LabelOcclusionManager {
    /**
     * Initializes the obstacle and placed label queues.
     */
    constructor() {
      this.obstacles = [];
      this.placedLabels = [];
    ...
    /**
     * Clears all tracking arrays before starting a new frame.
     */
    reset() {
    ...
    /**
     * Registers a rectangular physical body as an obstacle.
     * @param {number} x - The top-left x coordinate of the box.
     * @param {number} y - The top-left y coordinate of the box.
     * @param {number} w - The width of the box.
     * @param {number} h - The height of the box.
     * @param {number} [pad=6] - Padding around the box to ensure labels don't get too close.
     */
    ...
    /**
     * Checks if a proposed text bounding box collides with any obstacles or placed labels.
     * @param {Object} box - The proposed text bounds {left, top, right, bottom}.
    ...
    /**
     * Registers a successfully placed label to prevent future labels from overlapping it.
     * @param {Object} box - The bounds of the placed label {left, top, right, bottom}.
     */
    addPlacedLabel(box) {
  ...
  class SekharSimUI {
    /**
     * Initializes the SekharSimUI interface.
     * @param {Object} options - Configuration object for the UI.
     * @param {string} [options.title="Physics Simulation"] - Header title.
     * @param {string} [options.subtitle="Sekhar Teaching Hub"] - Header subtitle.
     * @param {Array<Object>} [options.sliders=[]] - Array of slider configurations.
     * @param {Array<Object>} [options.toggles=[]] - Array of toggle switch configurations.
     * @param {Array<Object>} [options.presets=[]] - Array of preset configurations.
     * @param {Array<string>} [options.hud=[]] - Array of keys for the HUD overlay.
     * @param {string} [options.theme="light"] - Initial theme ("light" or "dark").
     * @param {boolean} [options.loupe=false] - Whether the 3.75x Loupe magnifier is initially active.
     * @param {boolean} [options.zoomPan=true] - Whether to show the zoom/pan toolbar.
     * @param {Function} [options.onPlayChange] - Callback when play/pause is toggled.
     * @param {Function} [options.onReset] - Callback when reset button is clicked.
     * @param {Function} [options.onSpeedChange] - Callback when speed multiplier is changed.
     * @param {Function} [options.onControlChange] - Callback when a slider or toggle value changes.
     * @param {Function} [options.onResize] - Callback when the canvas container is resized.
     * @param {Function} [options.onThemeChange] - Callback when the theme changes.
     * @param {Function} [options.onZoomChange] - Callback when the canvas zoom/pan changes.
     */
    constructor(options = {}) {
      ...
      this.dropdownsConfig = options.dropdowns || [];
      this.segmentedConfig = options.segmented || [];
      this.actionsConfig = options.actions || [];
      this.numberInputsConfig = options.numberInputs || [];
      this.progressConfig = options.progress || [];
    ...
    _buildDOM() {
      ...
      // Dropdowns
      this.dropdownsConfig.forEach((d) => {
        this.values[d.id] = d.value !== undefined ? d.value : (d.options && d.options.length > 0 ? d.options[0].value : null);
        const card = document.createElement("div");
        card.className = "adw-action-row";
        let optionsHtml = d.options.map(opt => `<option value="${opt.value}" ${opt.value === this.values[d.id] ? "selected" : ""}>${opt.label}</option>`).join("");
        card.innerHTML = `
          <div class="row-title" style="margin-bottom: 4px;">${d.label}</div>
          <select class="adw-select" id="dropdown-${d.id}">${optionsHtml}</select>
        `;
        sidebar.appendChild(card);
      });

      // Segmented Controls
      this.segmentedConfig.forEach((seg) => {
        this.values[seg.id] = seg.value !== undefined ? seg.value : (seg.options && seg.options.length > 0 ? seg.options[0].value : null);
        const card = document.createElement("div");
        card.className = "adw-action-row";
        let btnsHtml = seg.options.map(opt => `<button class="segmented-tab ${opt.value === this.values[seg.id] ? "active" : ""}" data-id="${seg.id}" data-value="${opt.value}">${opt.label}</button>`).join("");
        card.innerHTML = `
          <div class="row-title" style="margin-bottom: 4px;">${seg.label}</div>
          <div class="segmented-tabs" id="segmented-${seg.id}">${btnsHtml}</div>
        `;
        sidebar.appendChild(card);
      });

      // Number Inputs
      this.numberInputsConfig.forEach((n) => {
        this.values[n.id] = Number(n.value || 0);
        const card = document.createElement("div");
        card.className = "adw-action-row";
        const unitStr = n.unit ? ` <span class="adw-subtitle-color" style="font-size:10px;">${n.unit}</span>` : "";
        card.innerHTML = `
          <div class="row-header">
            <span class="row-title">${n.label}</span>
            <div style="display:flex; align-items:center; gap:6px;">
              <input type="number" class="adw-number-input" id="num-${n.id}" value="${n.value}" min="${n.min!==undefined?n.min:''}" max="${n.max!==undefined?n.max:''}" step="${n.step||'any'}">
              ${unitStr}
            </div>
          </div>
        `;
      ...
      this.progressConfig.forEach((p) => {
        this.values[p.id] = Number(p.value || 0);
        const card = document.createElement("div");
        card.className = "adw-action-row";
        card.innerHTML = `
          <div class="adw-progress-container">
            <div class="row-header">
              <span class="row-title">${p.label}</span>
              <span class="adw-value-badge" id="badge-${p.id}">${this.values[p.id]}</span>
            </div>
            <input type="range" class="adw-progress-bar" id="progress-${p.id}" min="${p.min||0}" max="${p.max||100}" step="${p.step||1}" value="${this.values[p.id]}">
          </div>
        `;
        sidebar.appendChild(card);
      });
      ...
      if (this.actionsConfig.length > 0) {
        const card = document.createElement("div");
        card.className = "adw-action-row";
        this.actionsConfig.forEach((a) => {
          const btn = document.createElement("button");
          btn.className = "adw-action-btn";
          btn.id = `action-${a.id}`;
    ...
    _bindEvents() {
      ...
      // Dropdowns
      this.dropdownsConfig.forEach((d) => {
        const select = document.getElementById(`dropdown-${d.id}`);
        if (select) {
          select.onchange = (e) => {
            this.values[d.id] = e.target.value;
            this.listeners.controlChange.forEach((fn) => fn(d.id, this.values[d.id]));
          };
        }
      });
      ...
      if (typeof document !== "undefined") {
        document.querySelectorAll(".segmented-tab").forEach((btn) => {
          btn.onclick = (e) => {
            ...
            this.values[id] = val;
            const container = document.getElementById(`segmented-${id}`);
            if (container) {
              container.querySelectorAll(".segmented-tab").forEach(b => b.classList.remove("active"));
            }
            btn.classList.add("active");
      ...
      // Number Inputs
      this.numberInputsConfig.forEach((n) => {
        const input = document.getElementById(`num-${n.id}`);
        if (input) {
          input.oninput = (e) => {
            const val = parseFloat(e.target.value);
            if (!isNaN(val)) {
              this.values[n.id] = val;
              this.listeners.controlChange.forEach((fn) => fn(n.id, val));
            }
          };
        }
      });
      ...
      // Progress Scrubber
      this.progressConfig.forEach((p) => {
        const slider = document.getElementById(`progress-${p.id}`);
        const badge = document.getElementById(`badge-${p.id}`);
        if (slider) {
          slider.oninput = (e) => {
            const val = parseFloat(e.target.value);
            this.values[p.id] = val;
            if (badge) badge.textContent = val;
            this._updateProgressFill(slider);
            this.listeners.controlChange.forEach((fn) => fn(p.id, val));
          };
        }
      });
    ...
    _updateProgressFill(slider) {
      if (!slider) return;
      const min = parseFloat(slider.min || 0);
      const max = parseFloat(slider.max || 100);
      const val = parseFloat(slider.value);
      const pct = Math.max(0, Math.min(100, ((val - min) / (max - min)) * 100));
      slider.style.background = `linear-gradient(to right, var(--adw-accent-color) 0%, var(--adw-accent-color) ${pct}%, var(--adw-scale-track-bg) ${pct}%, var(--adw-scale-track-bg) 100%)`;
    }
    ...
    _updateAllSliderFills() {
      ...
      this.progressConfig.forEach((p) => {
        const slider = document.getElementById(`progress-${p.id}`);
        if (slider) this._updateProgressFill(slider);
      });
    }
     ...
     * @param {number} scale - The new zoom scale (clamped between 0.5 and 3.0).
     */
    setZoom(scale) {
    ...
    /**
     * Resets the canvas zoom scale to 1.0 (100%) and pan to 0,0.
     */
    resetZoom() {
      this.zoomScale = 1.0;
      this.panOffset = { x: 0, y: 0 };
    ...
    /**
     * Retrieves the current width of the canvas container.
    ...
    /**
     * Retrieves the current height of the canvas container.
    ...
    /**
     * Retrieves the current value of a slider or toggle.
     * @param {string} id - The ID of the control.
     * @returns {number|boolean} The current value.
     */
    get(id) {
    ...
    /**
     * Programmatically sets the value of a slider or toggle and updates the UI.
     * @param {string} id - The ID of the control.
     * @param {number|boolean} val - The new value.
     */
    set(id, val) {
      this.values[id] = val;
      if (typeof document !== "undefined") {
        ...
        const progSlider = document.getElementById(`progress-${id}`);
        const segContainer = document.getElementById(`segmented-${id}`);
        ...
        if (progSlider) {
          progSlider.value = val;
          if (badge) badge.textContent = val;
          this._updateProgressFill(progSlider);
        }
        if (segContainer) {
          segContainer.querySelectorAll(".segmented-tab").forEach(b => {
            if (b.dataset.value === String(val)) {
    ...
    /**
     * Applies a preset configuration map of values.
     * @param {Object} presetMap - Key-value pairs of control IDs to new values.
     */
    ...
    /**
     * Updates the on-canvas telemetry HUD overlay.
     * @param {string|Object} keyOrMap - Either a specific HUD key string, or an object of key-value pairs.
     * @param {string|number} [maybeVal] - The value if the first argument is a string key.
     */
    ...
    /** @param {Function} fn - Callback for play/pause toggle. */
    onPlayChange(fn) { this.listeners.playChange.push(fn); }
    /** @param {Function} fn - Callback for reset button click. */
    onReset(fn) { this.listeners.reset.push(fn); }
    /** @param {Function} fn - Callback for speed multiplier change (e.g. 1.0, 2.0). */
    onSpeedChange(fn) { this.listeners.speedChange.push(fn); }
    /** @param {Function} fn - Callback for control (slider/toggle) change. */
    onControlChange(fn) { this.listeners.controlChange.push(fn); }
    /** @param {Function} fn - Callback for canvas container resize. */
    onResize(fn) { this.listeners.resize.push(fn); }
    /** @param {Function} fn - Callback for theme change (light/dark). */
    onThemeChange(fn) { this.listeners.themeChange.push(fn); }
    /** @param {Function} fn - Callback for viewport zoom or pan change. */
